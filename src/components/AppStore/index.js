...
class AppStore extends Component {
  ...
  render() {
    ...
    const selectedTabApps = appsList.filter(
      eachApp => eachApp.category === activeTabId,
    )

    const searchedApp = selectedTabApps.filter(eachapp =>
    ...
    return (
      <div className="app_container">
        <div className="content_container">
          ...
          <ul className="tabslist_ul">
            {tabsList.map(eachTab => (
              <TabItem
                key={eachTab.tabId}
                eachTab={eachTab}
                selectTab={this.tabSelect}
                isSelectedTab={eachTab.tabId === activeTabId}
              />
            ))}
          </ul>

          <ul className="apps_container_ul">
            {searchedApp.map(eachApp => (
